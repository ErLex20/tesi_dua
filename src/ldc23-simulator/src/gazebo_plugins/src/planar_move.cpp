...
#include <gazebo_plugins/planar_move.hpp>
...
public:
  ...
  /// Initial pose
  ignition::math::v6::Pose3<double> init_pose;
};
...
void GazeboRosPlanarMove::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (impl_->publish_odom_tf_) {
    impl_->transform_broadcaster_ =
      std::make_shared<tf2_ros::StaticTransformBroadcaster>(impl_->ros_node_);
  ...
  // Set initial pose
  impl_->init_pose = impl_->model_->WorldPose();
}
  ...
  if (seconds_since_last_update >= update_period_) {
    ...
    model_->SetLinearVel(
      ignition::math::Vector3d(
        ...
        0));
    model_->SetAngularVel(ignition::math::Vector3d(0, 0, 1.57*target_cmd_vel_.angular.z));
